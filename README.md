...
### Status
Snackis UI is currently being ported from ```NCurses``` to ```Gtk+``` since it proved to cumbersome to implement desired functionality. The new version is nearing feature-parity, only the final pieces for posting are missing; and is provided as ```dist/gsnackis```. Once the conversion is complete, the previous UI will be deprecated.
