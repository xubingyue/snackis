...
namespace ui {
  ...
  static void update_history(PostForm &frm) {
    auto &ctx(frm.ctx);
    auto thread(get_thread_id(ctx, frm.thread.selected->val));
    frm.peers.clear();
    const auto width(frm.history.dim.w);
    Stream out;
    
    for (auto &post: last_posts(thread, 7)) {
      Peer by(get_peer_id(ctx, post.by_id));
      if (frm.peers.empty()) { frm.peers.insert(by); }
  ...
  PostForm::PostForm(View &view, Footer &ftr):
    ...
    peer.email.info = "Press Return to add/remove peer";
